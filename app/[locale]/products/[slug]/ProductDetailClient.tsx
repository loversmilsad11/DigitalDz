...
export default function ProductDetailClient({ product, locale, currency, labels }: ProductDetailClientProps) {
  ...
  const tabs = [
    ...
    { id: 'howto' as const,       label: isRtl ? 'طريقة الاستخدام' : 'Comment utiliser' },
    { id: 'reviews' as const,     label: isRtl ? 'التقييمات' : 'Avis' },
  ];

  /* ─── how-to steps ─────────────────────── */
  const howToSteps = isRtl
    ? [
    ...
    : [
        { n: '01', title: 'Ajouter au panier', desc: 'Cliquez sur "Ajouter au panier" ou "Acheter maintenant".' },
        { n: '02', title: 'Choisir le paiement', desc: 'Choisissez parmi: Baridimob, CCP, Edahabia ou CIB.' },
        { n: '03', title: 'Confirmer la commande', desc: 'Après confirmation, les clés vous seront envoyées instantanément.' },
        { n: '04', title: 'Profiter du produit', desc: 'Activez la clé sur la plateforme correspondante et profitez!' },
      ];

  /* ─── sample reviews ────────────────────── */
  const reviews = [
    { name: 'أحمد. ب', rating: 5, date: '15 مارس 2026', comment: isRtl ? 'منتج ممتاز، تسليم فوري. شكراً DigitalDZ!' : 'Excellent produit, livraison instantanée. Merci DigitalDZ!' },
    { name: 'كريم. م', rating: 4, date: '10 مارس 2026', comment: isRtl ? 'خدمة جيدة جداً والسعر مناسب. سأشتري مرة أخرى.' : 'Très bon service et prix adapté. Je rachèterai.' },
    { name: 'سارة. ل', rating: 4, date: '5 مارس 2026',  comment: isRtl ? 'تجربة تسوق سلسة، المفتاح شغّال 100%.' : 'Expérience fluide, la clé fonctionne à 100%.' },
  ];

  return (
    <>
      {/* ── Breadcrumb (client so hover works) ── */}
      <nav className="breadcrumb-nav" style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '2.5rem', fontSize: '0.85rem', color: 'var(--foreground-muted)' }}>
        <Link href={`/${locale}`} className="bc-link">{labels.home}</Link>
        <ChevronRight size={14} style={{ opacity: 0.4, transform: isRtl ? 'rotate(180deg)' : undefined }} />
        <Link href={`/${locale}/products`} className="bc-link">{labels.products}</Link>
        <ChevronRight size={14} style={{ opacity: 0.4, transform: isRtl ? 'rotate(180deg)' : undefined }} />
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.75rem' }}>
            ...
            <div className="glass-morphism" style={{
              ...
              <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.82rem', color: 'var(--foreground-muted)' }}>
                  <Clock size={14} />
                  {isRtl ? 'تسليم خلال دقائق' : 'Livraison en quelques minutes'}
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                ...
                {itemInCart && (
                  <Link href={`/${locale}/cart`} style={{ fontSize: '0.8rem', color: '#e879f9', textDecoration: 'none', fontWeight: 700 }}>
                    ({itemInCart.quantity} {isRtl ? 'في السلة' : 'dans le panier'})
                  </Link>
                )}
              </div>
            </div>
            ...
            {activeTab === 'description' && (
              <div>
                ...
                <div style={{ marginTop: '2rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem' }}>
                  {[
                    { label: isRtl ? 'نوع المنتج' : 'Type', value: isRtl ? 'مفتاح رقمي' : 'Clé numérique' },
                    { label: isRtl ? 'التسليم' : 'Livraison', value: isRtl ? 'فوري - إلكتروني' : 'Instantané - Électronique' },
                    { label: isRtl ? 'الفئة' : 'Catégorie', value: product.categoryName || '—' },
                    { label: isRtl ? 'العملة' : 'Devise', value: `${isRtl ? 'دينار جزائري' : 'Dinar algérien'} (${currency})` },
                  ].map((spec, i) => (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '2rem', padding: '1.5rem', borderRadius: '1.5rem', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)', flexWrap: 'wrap' }}>
                  <div style={{ textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: '0.8rem', color: 'var(--foreground-muted)' }}>128 {isRtl ? 'تقييم' : 'avis'}</div>
                  </div>
