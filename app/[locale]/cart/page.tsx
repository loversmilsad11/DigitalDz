        ...
        <FadeIn>
          <div className="container" style={{ padding: '6rem 2rem', textAlign: 'center' }}>
            ...
            <p style={{ color: 'var(--foreground-muted)', fontSize: '1.1rem', marginBottom: '3rem' }}>
              {isRtl ? 'لم تضف أي منتج بعد. تصفح متجرنا واستكشف أفضل العروض!' : "Vous n'avez pas encore ajouté de produits. Explorez notre boutique!"}
            </p>
            <Link href={`/${locale}/products`} className="btn-primary" style={{ fontSize: '1.1rem', padding: '1rem 2.5rem', borderRadius: '1.25rem', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '0.5rem' }}>
              <ShoppingBag size={20} />
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '3rem', flexWrap: 'wrap', gap: '1rem' }}>
            <div>
              ...
              <p style={{ color: 'var(--foreground-muted)' }}>
                {totalItems} {isRtl ? 'منتج في السلة' : 'produit(s) dans le panier'}
              </p>
            </div>
            <button
              onClick={clearCart}
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <p style={{ color: '#e879f9', fontWeight: 800, fontSize: '1.1rem' }}>
                        {(item.price * item.quantity).toLocaleString()} {isRtl ? 'د.ج' : 'DA'}
                      </p>
                    </div>
            ...
            <div className="glass-morphism" style={{
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '2rem' }}>
                {items.map(item => (
                  <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.9rem' }}>
                    ...
                    <span style={{ fontWeight: 700 }}>{(item.price * item.quantity).toLocaleString()} {isRtl ? 'د.ج' : 'DA'}</span>
                  </div>
                ))}
              </div>
              ...
              <div style={{ borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: '1.5rem', marginBottom: '2rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span className="title-font" style={{ fontSize: '1.1rem', fontWeight: 700 }}>{isRtl ? 'المجموع' : 'Total'}</span>
              ...
              >
                {isRtl ? 'إتمام الشراء' : 'Passer la commande'}
